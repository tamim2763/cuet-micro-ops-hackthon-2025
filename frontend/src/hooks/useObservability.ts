...
export function usePageTracking() {
  ...
  useEffect(() => {
    ...
    console.log(`[Page Tracking] ${fullPath}`, { traceId: getCurrentTraceId() });
  }, [location]);
}
...
export function useDownloadTracking() {
  return useCallback(
    (operation: 'initiate' | 'complete' | 'fail' | 'cancel', data?: Record<string, any>) => {
  ...
  return useCallback(
    async <T,>(operationName: string, apiCall: () => Promise<T>): Promise<T> => {
      ...
      try {
        ...
        const result = await createSpan(`api.${operationName}`, async (span) => {
          span.setAttributes({
        ...
        console.log(`[API Tracking] ${operationName} succeeded in ${duration.toFixed(2)}ms`);
        ...
        console.error(`[API Tracking] ${operationName} failed after ${duration.toFixed(2)}ms`, error);
...
export function useUserTracking() {
  return useCallback((user: { id: string; email?: string; username?: string } | null) => {
    if (user) {
      Sentry.setUser({
        id: user.id,
        email: user.email,
...
export function useErrorTracking() {
  return useCallback((error: Error | unknown, context?: string, data?: Record<string, any>) => {
    const traceId = getCurrentTraceId();
    ...
    console.error(`[Error Tracking] ${context || 'Error'}:`, error, { traceId, ...data });
  }, []);
}
